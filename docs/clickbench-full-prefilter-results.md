...
## DAG mid-cut localization

A subsequent prototype retains the selected DAG cut's exact left/right token
radius for every covered token. The one-bit 512-code summary is unchanged. Its
second pass uses fixed-shape AVX-512 point/range kernels to emit exact covered
code positions, then KMP or decoded `memmem` checks only the cut-bounded window
around each position. Sparse positions are mapped to rows by monotonic
interpolation over the existing offset array; no new input index is required.

All common cover shapes use const-specialized point/range kernels in both SIMD
passes. Across the same 30 needles, best-of-three totals are:
