...
/// [`Column::as_search_parts`], or by struct literal from data deserialized out
/// of storage. Like [`crate::Parts`], the fields are public and unchecked: the
/// search methods index `codes` by `code_offsets` without revalidating, so a
/// hand-built view must keep `code_offsets` monotonic and in bounds (a view
/// from `as_search_parts` always is).
#[derive(Copy, Clone, Debug)]
...
impl<O: Offset> SearchParts<'_, O> {
    ...
    /// Codes of row `r`: `codes[code_offsets[r]..code_offsets[r + 1]]`. The
    /// offsets are a caller-upheld invariant (monotonic, in bounds — see the
    /// type docs), so the conversion is the branchless [`Offset::as_usize`].
    #[inline]
